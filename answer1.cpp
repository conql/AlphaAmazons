...
inline void init() {
	for(int i = 0;i != 64;++i) {
		...
		for(auto x : {-1, 0, 1}) for(auto y : {-1, 0, 1}) if(x || y) {
			u64 e = 0;
			for(int a = x0, b = y0;;) {
				a += x, b += y;
				if((u32) a >= 8 || (u32) b >= 8) break;
				edge[i] |= 1ull << id(a, b);
				path[i][id(a, b)] = e;
				e |= 1ull << id(a, b);
			}
...
inline void KingMoveBfs(const board & x, DisArr & A) {
	...
	* A.a = x.a;
	...
	for(int x = i - 1;x >= 1;--x) A.a[x] ^= A.a[x - 1];
...
inline double eval(const board & x) {
	...
	for(int i = 1;i < k0.len;++i) p3 -= popc(range & k0.a[i]) / 6. * i;
	for(int i = 1;i < k1.len;++i) p3 += popc(range & k1.a[i]) / 6. * i;
	static double li[] = { 2. / (1 << 0), 2. / (1 << 1), 2. / (1 << 2), 2. / (1 << 3), 2. / (1 << 4), 2. / (1 << 5), };
	for(int i = 1;i < 6 && i < q0.len;++i) p2 += popc(q0.a[i]) * li[i];
	for(int i = 1;i < 6 && i < q1.len;++i) p2 -= popc(q1.a[i]) * li[i];
...
int SD;
struct pr { double score; board B; };
...
inline double search(const board & b, int dep, double alpha, double beta) {
	...
	// ans > beta : any > beta
	if(dep == SD) return b.eval();
	const double P = b.eval();
	const double AF = 0.7 + (dep == SD - 1) * 0.1;
	if(P + (1 - P) * (1 - AF) < alpha) return P;
	if(P * AF > beta) return P;
	if(dep == SD - 1) {
		lose = 1;
		return b.bestmoves(beta);
	}
	auto p = b.getmoves();
	if(p.begin() == p.end()) return -1e8;
...
inline void solve(int D, board b) {
	...
	stable_sort(v.begin(), v.end(), [&](move x, move y) { return b.Move(x).eval() < b.Move(y).eval(); });
	if(popc(b.c) == 8) {
		v.erase(remove_if(v.begin(), v.end(), [](move x){ return row(x.start) >= 3; }), v.end());
	}
	if(popc(b.c) > 8)
